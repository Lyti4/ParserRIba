"""
Парсер для магазина "Ашан".
Критично: Headers X-Region и X-Shop-Id обязательны.
"""
import asyncio
...
class AuchanProduct(BaseModel):
    """Модель продукта для Ашана."""
    name: str
    price_current: float
...
class AuchanParser(BaseParser):
    def __init__(self, context: BrowserContext, kb_path: str = "knowledge_base/auchan.md"):
        super().__init__(context, kb_path)
        self.kb = KBLoader().load_shop("auchan")
        ...
        if "X-Region" not in self.required_headers or "X-Shop-Id" not in self.required_headers:
            raise ValueError("AuchanParser: Headers X-Region и X-Shop-Id обязательны!")
    ...
    async def parse_category(self, url: str, max_pages: int = 1) -> List[AuchanProduct]:
        products = []
        ...
        js_code = """
        () => {
            const cards = document.querySelectorAll('article.product-card, div[class*="product"]');
            return Array.from(cards).map(card => {
                const nameEl = card.querySelector('h3, .product-name, a.product-link');
                const priceEl = card.querySelector('.price-current, [data-price]');
                const oldPriceEl = card.querySelector('.price-old, s');
                const unitPriceEl = card.querySelector('.price-per-kg, .unit-price');
                const weightEl = card.querySelector('.weight, .package-size');
        ...
        try:
            ...
            for item in raw_data:
                try:
                    product = AuchanProduct(**item, raw_data=item)
