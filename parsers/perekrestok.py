...
Критично: Только Playwright, cookies и X-Client-Id обязательны.
"""
import asyncio
...
class PerekrestokProduct(BaseModel):
    """Модель продукта для Перекрестка."""
    name: str
    price_current: float
    ...
    source: str = "perekrestok"
    raw_data: Optional[Dict[str, Any]] = None


class PerekrestokParser(BaseParser):
    def __init__(self, context: BrowserContext, kb_path: str = "knowledge_base/perekrestok.md"):
        super().__init__(context, kb_path)
        self.kb = KBLoader().load_shop("perekrestok")
        
        self.strategies = [
            ScrollStrategy(self.page, delay=1.5, steps=7),  # Больше шагов для поведенческой проверки
            LazyLoadStrategy(self.page, timeout=7000)
        ]
        
        self.required_headers = self.kb.headers.custom or {}
        if "X-Client-Id" not in self.required_headers:
            raise ValueError("PerekrestokParser: Header X-Client-Id обязателен!")

    async def navigate_to_category(self, url: str) -> bool:
        try:
            # Сохраняем cookies перед навигацией (критично для Перекрестка)
            ...
            await asyncio.sleep(3)  # Дольше ждем для поведенческой проверки
    ...
    async def is_blocked(self) -> bool:
        indicators = [
            ...
            "div[class*='behavioral-check']"
        ]
        for selector in indicators:
            if await self.page.query_selector(selector):
                self.logger.warning("Обнаружена защита/капча/поведенческая проверка")
                return True
        return False

    async def parse_category(self, url: str, max_pages: int = 1) -> List[PerekrestokProduct]:
        products = []
        ...
        try:
            ...
            for item in raw_data:
                try:
                    product = PerekrestokProduct(**item, raw_data=item)
    ...
    async def handle_captcha(self) -> bool:
        self.logger.info("Попытка обхода капчи/поведенческой проверки...")
        await asyncio.sleep(10)  # Дольше ждем
