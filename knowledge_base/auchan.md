# 📘 Knowledge Base: Ашан (auchan.ru)

## ℹ️ Общая информация

- **Название сети**: Ашан
- **Базовый URL**: `https://auchan.ru`
...
|-----------|-----|------------|
| Рыба и морепродукты | `https://auchan.ru/catalog/ryba-moreprodukty` | Основная категория |
| Копченая и соленая рыба | `https://auchan.ru/catalog/ryba-kopchenaya-solenaya` | Подкатегория |
| Икра и рыбные деликатесы | `https://auchan.ru/catalog/ikra-rybnye-delikatesy` | Премиум сегмент |
| Рыбные консервы | `https://auchan.ru/catalog/konservy-rybnye` | Консервированная продукция |
| Морепродукты | `https://auchan.ru/catalog/moreprodukty` | Креветки, мидии и др. |

### Дополнительные URL

- **Карточка товара**: `https://auchan.ru/product/{product_id}`
- **API эндпоинты** (потенциально):
  - `GET https://auchan.ru/api/products/` - список товаров
  - Параметры: `categoryId`, `page`, `size`, `sort`
...
### Специфичные заголовки для Ашана

```python
{
    # Ашан ТРЕБУЕТ указания региона
...
- **Сохранение сессии**: Обязательно (`auchan_session.json`)
...
`https://auchan.ru/product/12345678/ryba-losos-file-ohlazhdennoe-1kg`
...
```json
{
  ...
  "weight": "1 кг",
  "brand": "Каждый День",
  "url": "https://auchan.ru/product/12345678",
  "image_url": "https://auchan.ru/media/products/12345678.jpg"
}
```
...
|------|-----------|-------|
| 2024-01-XX | Начальная версия на основе parsers/auchan.py | AI Assistant |
| ... | ... | ... |
