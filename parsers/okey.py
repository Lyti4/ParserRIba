...
class OkeyParser(BaseParser):
    """Парсер для магазина Окей"""
    
    def __init__(self):
        super().__init__(
            store_name="Окей",
            base_url="https://www.okey.ru"
        )
        self.categories = [
    ...
    async def parse_fish_products(self) -> List[FishProduct]:
        """Парсинг рыбных товаров из Окей"""
        products = []
        ...
        logger.info(f"Всего найдено товаров в Окей: {len(products)}")
        return products
    ...
    def _parse_product_list(self, html: str, category_url: str) -> List[FishProduct]:
        ...
        product_cards = soup.select('.product-card, .catalog-item, [data-product], .goods-item, .product-tile')
    ...
    def _extract_product(self, card, category_url: str) -> FishProduct:
        ...
        elif "икр" in category_url.lower() or "деликатес" in category_url.lower():
        ...
        elif "рыб" in category_url.lower():
