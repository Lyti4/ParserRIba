...
class ParseMetrics(BaseModel):
    ...
    parse_duration_ms: int = Field(default=0, description="Длительность парсинга в мс")
    page_load_time_ms: Optional[int] = Field(None, description="Время загрузки страницы в мс")
    total_requests: int = Field(default=0, description="Количество HTTP запросов")
    blocked_resources: int = Field(default=0, description="Заблокировано ресурсов (изображения, трекеры)")
    retries_count: int = Field(default=0, description="Количество повторных попыток")
...
class ParseResult(BaseModel):
    ...
    metrics: Optional[ParseMetrics] = Field(None, description="Метрики производительности")
