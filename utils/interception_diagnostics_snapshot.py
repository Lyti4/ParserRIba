"""Safe snapshot writer for interception diagnostics."""
...
def build_interception_diagnostics_snapshot(result: dict[str, Any]) -> dict[str, Any]:
    """Build compact safe interception diagnostics payload."""
...
def write_interception_diagnostics_snapshot(result: dict[str, Any], output_dir: Path | str) -> Path:
    """Write compact interception diagnostics snapshot and return its path."""
    target_dir = Path(output_dir)
    target_dir.mkdir(parents=True, exist_ok=True)
    category = _safe_name(str(result.get("category") or "category"))
    path = target_dir / f"pyaterochka_{category}_interception_diagnostics.json"
    payload = build_interception_diagnostics_snapshot(result)
